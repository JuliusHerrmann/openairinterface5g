...
bool parse_ue_sim_param(config_setting_t *ue_setting, int user_id, usim_data_conf_t *u) {
  ...
  rc = config_setting_lookup_string(ue_param_setting, USIM_API_K, &u->usim_api_k);
  if (rc != 1) {
...
void gen_usim_data(usim_data_conf_t *u, usim_data_t *usim_data,
    ...
    usim_data->imsi.u.num.parity = get_msin_parity(u->msin, conf->mcc, conf->mnc);
    ...
    memset(usim_data->securityctx.Kasme.value, 0, usim_data->securityctx.Kasme.length);
    ...
    memset(usim_data->securityctx.ulNAScount.value, 0, usim_data->securityctx.ulNAScount.length);
    ...
    memset(usim_data->securityctx.dlNAScount.value, 0, usim_data->securityctx.dlNAScount.length);
    ...
    for (int i = 0; i < user_plmns->operators.size; i++) {
      ...
      strncpy((char *)usim_data->pnn[i].fullname.value, record.fullname, usim_data->pnn[i].fullname.length);
      usim_data->pnn[i].shortname.type = USIM_PNN_SHORTNAME_TAG;
      usim_data->pnn[i].shortname.length = strlen(record.shortname);
      strncpy((char *)usim_data->pnn[i].shortname.value, record.shortname, usim_data->pnn[i].shortname.length);
    ...
    if (user_plmns->operators_controlled.size > 0) {
      for (int i = 0; i < user_plmns->operators_controlled.size; i++) {
        ...
        usim_data->oplmn[i].AcT = (USIM_ACT_GSM | USIM_ACT_UTRAN | USIM_ACT_EUTRAN);
      }
    }
    /*
     * EPS Location Information
     */
    usim_data->epsloci.guti.gummei.plmn = networks.items[hplmn_index].plmn;
    ...
    if (hex_string_to_hex_value(usim_data->keys.usim_api_k, u->usim_api_k, USIM_API_K_SIZE) == -1
        || hex_string_to_hex_value(usim_data->keys.opc, u->opc, OPC_SIZE) == -1) {
