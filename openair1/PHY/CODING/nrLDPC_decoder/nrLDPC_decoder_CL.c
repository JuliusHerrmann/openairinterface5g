...
void set_compact_BG(int Zc,short BG){
  ...
  printf("\nZc = %d BG = %d\n", Zc, BG);
  ocl.runtime[0].dev_h_compact1 =
      clCreateBuffer(ocl.runtime[0].context, CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY, memorySize_h_compact1, NULL, (cl_int *)&rt);
  AssertFatal(rt == CL_SUCCESS, "Error %d creating buffer dev_h_compact1 for platform %i \n", (int)rt, 0);
  ocl.runtime[0].dev_h_compact2 =
      clCreateBuffer(ocl.runtime[0].context, CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY, memorySize_h_compact2, NULL, (cl_int *)&rt);
...
int ldpc_autoinit(void) {   // called by the library loader 
  ...
  for (int i=0 ; i<(int)num_platforms_found ; i++) {
      ...
      rt = clGetDeviceIDs(platforms[i],
                          CL_DEVICE_TYPE_ALL,
                          sizeof(ocl.runtime[i].devices) / sizeof(cl_device_id),
                          ocl.runtime[i].devices,
                          &(ocl.runtime[i].num_devices));
      ...
      for (int j = 0; j < ocl.runtime[i].num_devices; j++) {
        ...
        rt = clGetDeviceInfo(ocl.runtime[i].devices[j],
                             CL_DEVICE_MAX_COMPUTE_UNITS,
                             sizeof(ocl.runtime[i].ocldev[j].max_CU),
                             &(ocl.runtime[i].ocldev[j].max_CU),
                             NULL);
        ...
        rt = clGetDeviceInfo(ocl.runtime[i].devices[j],
                             ...
                             sizeof(ocl.runtime[i].ocldev[j].max_WID),
                             &(ocl.runtime[i].ocldev[j].max_WID),
                             NULL);
        ...
        rt = clGetDeviceInfo(ocl.runtime[i].devices[j],
                             ...
                             sizeof(ocl.runtime[i].ocldev[j].max_WID) * sizeof(size_t),
                             ocl.runtime[i].ocldev[j].max_WIS,
                             NULL);
...
//    e_nrLDPC_outMode outMode = p_decParams->outMode;
    ...
    for (int ii = 0; ii < MAX_ITERATION; ii++) {
      ...
      if (ii == 0) {
        ...
        rt = clEnqueueNDRangeKernel(ocl.runtime[0].queue[0],
                                    ocl.runtime[0].kernels[0].cnp_kernel_1st,
                                    2,
                                    NULL,
                                    global_item_sizek0,
                                    local_item_sizek,
        ...
        rt = clEnqueueNDRangeKernel(ocl.runtime[0].queue[0],
                                    ocl.runtime[0].kernels[0].cnp_kernel,
                                    2,
                                    NULL,
                                    global_item_sizek0,
                                    local_item_sizek,
