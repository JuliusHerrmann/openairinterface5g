...
#define TEST_ASSERT_SUCCESS(val, msg, ...)                                                                 \
  do {                                                                                                     \
    typeof(val) _val = (val);                                                                              \
    if (!(_val == 0)) {                                                                                    \
      printf("TestCase %s() line %d failed (err %d): " msg "\n", __func__, __LINE__, _val, ##__VA_ARGS__); \
      return TEST_FAILED;                                                                                  \
    }                                                                                                      \
  } while (0)

#define TEST_ASSERT_FAIL(val, msg, ...) TEST_ASSERT_SUCCESS(!(val), msg, ##__VA_ARGS__)

#define TEST_ASSERT_NOT_NULL(val, msg, ...)                                                        \
  do {                                                                                             \
    if ((val) == NULL) {                                                                           \
      printf("TestCase %s() line %d failed (null): " msg "\n", __func__, __LINE__, ##__VA_ARGS__); \
      return TEST_FAILED;                                                                          \
    }                                                                                              \
  } while (0)
