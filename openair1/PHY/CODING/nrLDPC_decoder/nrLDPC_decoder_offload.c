...
typedef int(test_case_function)(struct active_device *ad, struct test_op_params *op_params);
...
check_dev_cap(const struct rte_bbdev_info *dev_info)
{
  ...
  for (i = 0; op_cap->type != RTE_BBDEV_OP_NONE; ++i, ++op_cap) {
    ...
    if (op_cap->type == RTE_BBDEV_OP_LDPC_DEC) {
      const struct rte_bbdev_op_cap_ldpc_dec *cap = &op_cap->cap.ldpc_dec;

      if (!flags_match(RTE_BBDEV_LDPC_ITERATION_STOP_ENABLE, cap->capability_flags)) {
...
static int create_mempools(struct active_device *ad,
                           int socket_id,
                           ...
                           t_nrLDPCoffload_params *p_offloadParams)
{
  ...
  snprintf(pool_name, sizeof(pool_name), "%s_pool_%u", op_type_str, ad->dev_id);
  mp = rte_bbdev_op_pool_create(pool_name, op_type, ops_pool_size, OPS_CACHE_SIZE, socket_id);
  ...
  if (nb_segments > 0) {
    ...
    mp = create_mbuf_pool(p_offloadParams->E, ad->dev_id, socket_id, mbuf_pool_size, "in");
    TEST_ASSERT_NOT_NULL(mp,
  ...
  if (nb_segments > 0) {
    mbuf_pool_size = optimal_mempool_size(ops_pool_size * nb_segments);
    mp = create_mbuf_pool(data_len, ad->dev_id, socket_id, mbuf_pool_size, "hard_out");
    TEST_ASSERT_NOT_NULL(mp,
 ...
 */
#ifdef RTE_LIBRTE_PMD_BBDEV_FPGA_LTE_FEC
  if ((get_init_device() == true) && (!strcmp(info->drv.driver_name, FPGA_LTE_PF_DRIVER_NAME))) {
    ...
    TEST_ASSERT_SUCCESS(ret, "Failed to configure 4G FPGA PF for bbdev %s", info->dev_name);
  }
#endif
#ifdef RTE_LIBRTE_PMD_BBDEV_FPGA_5GNR_FEC
  if ((get_init_device() == true) && (!strcmp(info->drv.driver_name, FPGA_5GNR_PF_DRIVER_NAME))) {
    ...
    TEST_ASSERT_SUCCESS(ret, "Failed to configure 5G FPGA PF for bbdev %s", info->dev_name);
  }
#endif
  ...
  TEST_ASSERT(queue_id != 0, "ERROR Failed to configure any queues on dev %u", dev_id);
...
populate_active_devices(void)
{
  ...
  RTE_BBDEV_FOREACH(dev_id)
  {
    ...
    if (check_dev_cap(&info)) {
      printf("Device %d (%s) does not support specified capabilities\n", dev_id, info.dev_name);
      continue;
    }
  ...
  for (i = 0; i < n; ++i) {
    /*    if (ldpc_dec->code_block_mode == 0) {
            ...
            ops[i]->ldpc_dec.tb_params.r =
                    ...
                    printf("code block ea %d eb %d c %d cab %d r
...
{
  int ret = 0;
  if (op_type == RTE_BBDEV_OP_TURBO_DEC || op_type == RTE_BBDEV_OP_LDPC_DEC)
  ...
  else if (op_type == RTE_BBDEV_OP_TURBO_ENC || op_type == RTE_BBDEV_OP_LDPC_ENC)
...
int32_t nrLDPC_decod_offload(t_nrLDPC_dec_params *p_decParams,
                             uint8_t harq_pid,
                             uint8_t ulsch_id,
                             uint8_t C,
                             uint8_t rv,
  ...
  */
  ...
  switch (mode) {
    case 0:
      ...
      f_ret = init_test_op_params(op_params, op_type, 0, 0, ad->ops_mempool, 1, get_num_ops(), get_num_lcores());
      if (f_ret != TEST_SUCCESS) {
      ...
      for (i = 0; i < ad->nb_queues; ++i) {
        ...
        TEST_ASSERT_NOT_NULL(t_params,
                             ...
                             RTE_ALIGN(sizeof(struct thread_params), RTE_CACHE_LINE_SIZE));
      }
    ...
    case 1:
      ...
      // start_time1 = rte_rdtsc_precise();
      for (type = DATA_INPUT; type < 3; type += 2) {
        ret = init_op_data_objs(*queue_ops[type],
                                p_llr,
                                p_offloadParams->E,
                                m_head[type],
                                mbuf_pools[type],
