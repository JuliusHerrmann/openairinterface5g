...
void applyGtoright(const t_nrPolar_params *pp,decoder_node_t *node) {
  ...
  if (node->right->all_frozen == 0) {  
    ...
    if (avx2mod == 0) {
      ...
      ((simde__m128i *)alpha_r)[0] =
          simde_mm_subs_epi16(((simde__m128i *)alpha_v)[1],
                              simde_mm_sign_epi16(((simde__m128i *)alpha_v)[0], ((simde__m128i *)betal)[0]));
    }
    else if (avx2mod == 4) {
      ((simde__m64 *)alpha_r)[0] =
          simde_mm_subs_pi16(((simde__m64 *)alpha_v)[1], simde_mm_sign_pi16(((simde__m64 *)alpha_v)[0], ((simde__m64 *)betal)[0]));
    }
    else
      {
        int temp_alpha_r;
    ...
    if (avx2mod == 0) {
      ...
      for (int i=0;i<avx2len;i++) {
          ((simde__m256i *)betav)[i] =
              simde_mm256_or_si256(simde_mm256_cmpeq_epi16(((simde__m256i *)betar)[i], ((simde__m256i *)betal)[i]), allones);
      }
    }
    else if (avx2mod == 8) {
      ((simde__m128i *)betav)[0] =
          simde_mm_or_si128(simde_mm_cmpeq_epi16(((simde__m128i *)betar)[0], ((simde__m128i *)betal)[0]), *((simde__m128i *)all1));
    }
    else if (avx2mod == 4) {
      ((simde__m64 *)betav)[0] =
          simde_mm_or_si64(simde_mm_cmpeq_pi16(((simde__m64 *)betar)[0], ((simde__m64 *)betal)[0]), *((simde__m64 *)all1));
    }
    else
      {
      for (int i = 0; i < node->Nv / 2; i++) {
