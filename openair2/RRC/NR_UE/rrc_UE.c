...
void *rrc_nrue(void *notUsed)
{
  ...
  case NR_RRC_MAC_SBCCH_DATA_IND:
    ...
    nr_rrc_ue_decode_NR_SBCCH_SL_BCH_Message(instance, sbcch->gnb_index,sbcch->frame, sbcch->slot, sbcch->sdu,
                                             sbcch->sdu_size, sbcch->rx_slss_id,
                                             &NR_UE_rrc_inst[instance]);
    break;
...
void start_sidelink(instance_t instance)
{
  ...
  if (get_softmodem_params()->sl_mode == 2) {
    ...
    rrc_ue_process_sidelink_Preconfiguration(rrc, get_softmodem_params()->sync_ref, instance);
