...
//  L2 Abstraction Layer
static nr_dci_format_t handle_dci(module_id_t module_id, frame_t frame, int slot, fapi_nr_dci_indication_pdu_t *dci)
{
  return nr_ue_process_dci_indication_pdu(module_id, frame, slot, dci);
}
...
static uint32_t nr_ue_dl_processing(nr_downlink_indication_t *dl_info)
{
  ...
  if (dl_info && dl_info->dci_ind && dl_info->dci_ind->number_of_dcis) {
    ...
    for (int i = 0; i < dl_info->dci_ind->number_of_dcis; i++) {
      ...
      nr_dci_format_t dci_format = handle_dci(dl_info->module_id, dl_info->frame, dl_info->slot, dl_info->dci_ind->dci_list + i);
