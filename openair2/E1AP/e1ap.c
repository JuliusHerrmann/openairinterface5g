...
static int fill_BEARER_CONTEXT_SETUP_REQUEST(e1ap_bearer_setup_req_t *const bearerCxt, E1AP_E1AP_PDU_t *pdu)
{
  ...
  for (pdu_session_to_setup_t *i = bearerCxt->pduSession; i < bearerCxt->pduSession + bearerCxt->numPDUSessions; i++) {
    ...
    for (DRB_nGRAN_to_setup_t *j = i->DRBnGRanList; j < i->DRBnGRanList + i->numDRB2Setup; j++) {
      ...
      /* SDAP config */
      ieC6_1_1->sDAP_Configuration.defaultDRB = j->sdap_config.defaultDRB;
      ...
      case E1AP_ProtocolIE_ID_id_System_BearerContextSetupRequest:
        ...
        for (int i = 0; i < pdu2SetupList->list.count; i++) {
          ...
          for (int j = 0; j < drb2SetupList->list.count; j++) {
            ...
            /* SDAP */
            drb->sdap_config.defaultDRB = drb2Setup->sDAP_Configuration.defaultDRB;
...
static int fill_BEARER_CONTEXT_MODIFICATION_REQUEST(e1ap_bearer_setup_req_t *const bearerCxt, E1AP_E1AP_PDU_t *pdu)
{
  ...
  for (pdu_session_to_mod_t *i = bearerCxt->pduSessionMod; i < bearerCxt->pduSessionMod + bearerCxt->numPDUSessionsMod; i++) {
    ...
    for (DRB_nGRAN_to_mod_t *j = i->DRBnGRanModList; j < i->DRBnGRanModList + i->numDRB2Modify; j++) {
      ...
      if (j->pdcp_config.pDCP_Reestablishment == true) {
...
{
  pdcp_config->pDCP_SN_Size_UL = E1AP_PDCP_SN_Size_s_18;
  pdcp_config->pDCP_SN_Size_DL = E1AP_PDCP_SN_Size_s_18;
  pdcp_config->discardTimer = E1AP_DiscardTimer_infinity;
  pdcp_config->reorderingTimer = E1AP_T_Reordering_ms100;
  pdcp_config->rLC_Mode = E1AP_RLC_Mode_rlc_am;
}
