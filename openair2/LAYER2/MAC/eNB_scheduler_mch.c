...
int8_t get_mbsfn_sf_alloction(module_id_t module_idP, uint8_t CC_id, uint8_t mbsfn_sync_area)
{
    ...
    if (mbsfn_sync_area > MAX_MBSFN_AREA) {
      ...
      return -1;
    } else if (RC.mac[module_idP]->common_channels[CC_id].non_mbsfn_SubframeConfig != NULL) {
...
static int check_nonMBSFN_sf(frame_t frameP,COMMON_channels_t *cc,int sf){
   uint32_t non_mbsfn_SubframeConfig = (cc->non_mbsfn_SubframeConfig->subframeAllocation_r14.buf[0] << 1
                                        | cc->non_mbsfn_SubframeConfig->subframeAllocation_r14.buf[0] >> 7);
   if (frameP % (4 << cc->non_mbsfn_SubframeConfig->radioFrameAllocationPeriod_r14)
       == cc->non_mbsfn_SubframeConfig->radioFrameAllocationOffset_r14) {
...
int schedule_MBMS_NFAPI(module_id_t module_idP, uint8_t CC_id, frame_t frameP, sub_frame_t subframeP)
{
    ...
    unsigned char header_len_mcch = 0, header_len_msi = 0, header_len_mtch = 0, header_len_mtch_temp = 0, header_len_mcch_temp = 0,
                  header_len_msi_temp = 0;
    int ii = 0, msi_pos = -1;
    int mcch_mcs = -1;
    int shifted_sf = 0;
    ...
    for (i = 0; i < cc->num_active_mbsfn_area; i++) {
      ...
      if ((j = get_mbsfn_sf_alloction(module_idP, CC_id, i)) == -1) {
        ...
        if(cc->non_mbsfn_SubframeConfig){
            ...
            LOG_D(MAC,
                  "frameP %d subframe %d period %d alloc_offset %d mcch_mcs %d mcch_period %d mcch_offset %ld buf %x "
                  "mch_scheduling_period %d\n",
                  frameP,
                  subframeP,
                  period,
                  alloc_offset,
                  ...
                  mcch_period,
                  mcch_offset,
                  (cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0]),
            ...
            if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
              if (x == cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9 + shifted_sf) {
                ...
                if (shifted_sf == 0)
                  mbms_mch_i++;
                LOG_D(MAC,
                      "MSP, frameP %d subframeP %d msi_pos(%d) mbms_mch_i %d shifted_sf %d\n",
                      frameP,
                      subframeP,
                      msi_pos,
                      mbms_mch_i,
            ...
            if ((msi_flag == 1) || (mcch_flag == 1) || (mtch_flag == 1)) {
               ...
               if (msi_flag == 1) {
                  if ((/*AJ*/ (/*V*/ (/*U*/ (frameP % (4 << cc->commonSF_AllocPeriod_r9))) / 8)
                       % ((8 << cc->pmch_Config[mbms_mch_i]->mch_SchedulingPeriod_r9) / 8))
                      != 0) {
                       msi_flag = 0;
               ...
               LOG_D(MAC,
                     "[eNB %d] CC_id %d Frame %d Subframe %d: sync area %d SF alloc %d: msi flag %d, mcch flag %d, mtch flag %d, x "
                     "%d\n",
                     module_idP,
                     CC_id,
                     frameP,
            ...
            if (frameP % mbsfn_period == cc->mbsfn_SubframeConfig[j]->radioframeAllocationOffset) { // MBSFN frameP
             if (cc->mbsfn_SubframeConfig[j]->subframeAllocation.present
                 == LTE_MBSFN_SubframeConfig__subframeAllocation_PR_oneFrame) { // one-frameP format
                 ...
                 if (frameP % mch_scheduling_period == cc->mbsfn_SubframeConfig[j]->radioframeAllocationOffset) {
                       ...
                       LOG_D(MAC,
                             "[eNB %d] CC_id %d Frame %d subframeP %d : sync area %d sf allocation pattern %d sf alloc %x msi pos "
                             "is %d "
                             "\n",
                             module_idP,
                             CC_id,
                 ...
                 switch (subframeP) {
                       case 1:
                         if (cc->tdd_Config == NULL) {
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_FDD_SF1)
                               == MBSFN_FDD_SF1) {
                             if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
                             ...
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_FDD_SF1)
                                     == MBSFN_FDD_SF1)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 2:
                         if (cc->tdd_Config == NULL) {
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_FDD_SF2)
                               == MBSFN_FDD_SF2) {
                             if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
                               if (x == cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9) {
                                 msi_pos = 2;
                             ...
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_FDD_SF2)
                                     == MBSFN_FDD_SF2)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 3:
                         if (cc->tdd_Config != NULL) {
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_TDD_SF3)
                               == MBSFN_TDD_SF3) {
                             if (msi_pos == 1) {
                               msi_flag = 1;
                             }
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_TDD_SF3)
                                     == MBSFN_TDD_SF3)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                           ...
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_FDD_SF3)
                               == MBSFN_FDD_SF3) {
                             if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
                               if (x == cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9) {
                                 msi_pos = 3;
                             ...
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_FDD_SF3)
                                     == MBSFN_FDD_SF3)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 4:
                         if (cc->tdd_Config != NULL) {
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_TDD_SF4)
                               == MBSFN_TDD_SF4) {
                             if (msi_pos == 2) {
                               msi_flag = 1;
                             }
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_TDD_SF4)
                                     == MBSFN_TDD_SF4)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 6:
                         if (cc->tdd_Config == NULL) {
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_FDD_SF6)
                               == MBSFN_FDD_SF6) {
                             if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
                               if (x == cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9) {
                                 msi_pos = 4;
                             ...
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_FDD_SF6)
                                     == MBSFN_FDD_SF6)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 7:
                         if (cc->tdd_Config != NULL) { // TDD
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_TDD_SF7)
                               == MBSFN_TDD_SF7) {
                             if (msi_pos == 3) {
                               msi_flag = 1;
                             }
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_TDD_SF7)
                                     == MBSFN_TDD_SF7)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                           ...
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_FDD_SF7)
                               == MBSFN_FDD_SF7) {
                             if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
                               if (x == cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9) {
                                 msi_pos = 5;
                             ...
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_FDD_SF7)
                                     == MBSFN_FDD_SF7)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 8:
                         if (cc->tdd_Config != NULL) { // TDD
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_TDD_SF8)
                               == MBSFN_TDD_SF8) {
                             if (msi_pos == 4) {
                               msi_flag = 1;
                             }
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_TDD_SF8)
                                     == MBSFN_TDD_SF8)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                           ...
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_FDD_SF8)
                               == MBSFN_FDD_SF8) {
                             if (cc->pmch_Config[mbms_mch_i + 1] != NULL) {
                               if (x == cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9) {
                                 msi_pos = 6;
                             ...
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_FDD_SF8)
                                     == MBSFN_FDD_SF8)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                       ...
                       case 9:
                         if (cc->tdd_Config != NULL) {
                           if ((cc->mbsfn_SubframeConfig[j]->subframeAllocation.choice.oneFrame.buf[0] & MBSFN_TDD_SF9)
                               == MBSFN_TDD_SF9) {
                             if (msi_pos == 5) {
                               msi_flag = 1;
                             }
                             if ((frameP % mcch_period == cc->mbsfn_AreaInfo[i]->mcch_Config_r9.mcch_Offset_r9)
                                 && ((cc->mbsfn_AreaInfo[i]->mcch_Config_r9.sf_AllocInfo_r9.buf[0] & MBSFN_TDD_SF9)
                                     == MBSFN_TDD_SF9)) {
                               mcch_flag = 1;
                             }
                             mtch_flag = 1;
                 ...
                 if ((msi_flag == 1) || (mcch_flag == 1) || (mtch_flag == 1)) {
                       ...
                       LOG_D(MAC,
                             "[eNB %d] CC_id %d Frame %d Subframe %d: sync area %d SF alloc %d: msi flag %d, mcch flag %d, mtch "
                             "flag %d "
                             "x %d\n",
                             module_idP,
                             CC_id,
    ...
    if (msi_flag == 1 && cc->mbms_SessionList[mbms_mch_i]) {
      ...
      for (k = 0; k < num_mtch; k++) { // loop for all session in this MCH (MCH[0]) at this moment
            ...
            ((MSI_ELEMENT *)msi_ptr)->lcid =
                cc->mbms_SessionList[mbms_mch_i]->list.array[k]->logicalChannelIdentity_r9; // mtch_lcid;
            ...
            if (msi_pmch_stop >= num_sf_alloc /*&& msi_pmch_stop <=cc->pmch_Config[0]->sf_AllocEnd_r9*/) {
             ((MSI_ELEMENT *)msi_ptr)->stop_sf_MSB =
                 (((msi_pmch_stop <= cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9 ? msi_pmch_stop
                                                                                 : cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9)
                   >> 8)
                  & 0x7f);
             ((MSI_ELEMENT *)msi_ptr)->stop_sf_LSB =
                 ((msi_pmch_stop <= cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9 ? msi_pmch_stop
                                                                                : cc->pmch_Config[mbms_mch_i]->sf_AllocEnd_r9)
                  & 0xff);
             msi_pmch_stop =
      ...
      LOG_D(MAC,
            "[eNB %d] CC_id %d Frame %d : MSI->MCH, length of MSI is %d bytes TBS %d, bytes in buffer %d stop_sf_LSB %d msi_sfs %d "
            "cc->pmch_Config[0]->sf_AllocEnd_r9 %ld\n",
            module_idP,
            CC_id,
            frameP,
            msi_length,
      ...
      memcpy((char *)&mch_buffer[sdu_length_total], msi_control_element, msi_length);
    ...
    if (mcch_flag == 1) {
      ...
      if (!cc->non_mbsfn_SubframeConfig) {
            ...
            if (mcch_sdu_length > 0) {
             ...
             memcpy((char *)&mch_buffer[sdu_length_total] + 1, &cc->MCCH_pdu.payload[0], mcch_sdu_length);
    ...
    TBS = get_TBS_DL(/*cc->pmch_Config[0]->dataMCS_r9*/ cc->MCH_pdu.mcs, to_prb(cc->mib->message.dl_Bandwidth));
    ...
    if (mtch_flag == 1 && cc->mbms_SessionList[0] && cc->mbms_SessionList[0]->list.array[0]) {
      ...
      if (msi_flag == 1 && buffer_pointer != NULL) {
        ...
        for (k = 0; k < num_mtch; k++) { // loop for all session in this MCH (MCH[0]) at this moment
          ...
          LOG_I(MAC,
                "frameP %d, subframeP %d LCID %d rlc_status.bytes_in_buffer %d stop_sf_LSB %d stop_sf_MSB %d msi_pmch_stop %d "
                "sf_AllocEnd_r9 %ld, msi_length %d\n",
                frameP,
                subframeP,
                ((MSI_ELEMENT *)msi_ptr)->lcid,
        ...
        memcpy((char *)buffer_pointer, msi_control_element, msi_length);
      }
      ...
      // TODO not sure whether msi and mch MCH should be precluded ... keep in mind this konditions just in case if
      // ((rlc_status.bytes_in_buffer > 0 &&  msi_pmch_stop > 0) && ((msi_flag!=1 || mcch_flag!=1))
      if ((rlc_status.bytes_in_buffer > 0
           && msi_pmch_stop > 0) /*&& ((msi_flag!=1 || mcch_flag!=1))*/ /*|| (rlc_status.bytes_in_buffer > 0 && (msi_flag==1 ||
                                                                           mcch_flag==1))*/
          /*|| msi_sfs > cc->pmch_Config[0]->sf_AllocEnd_r9 */ /*msi_pmch_stop>=num_sf_alloc*/) {
        LOG_I(MAC,
              "[eNB %d][MBMS USER-PLANE], CC_id %d, Frame %d, Subframe %d MTCH->MCH, Requesting %d bytes from RLC (header len mtch "
              "%d) rlc_status.bytes_in_buffer %d, LCID %ld\n",
              module_idP,
              CC_id,
              frameP,
              subframeP,
    ...
    if ((sdu_length_total + header_len_msi + header_len_mcch + header_len_mtch) > 0) {
      ...
      if ((TBS - header_len_mtch - header_len_mcch - header_len_msi - sdu_length_total) < 0) {
        LOG_E(MAC,
              "Error in building MAC PDU, TBS %d < PDU %d \n",
              TBS,
              header_len_mtch + header_len_mcch + header_len_msi + sdu_length_total);
        return 0;
      ...
      LOG_D(MAC,
            ...
            cc->MCH_pdu.mcs,
            cc->MCH_pdu.mcch_active,
            cc->MCH_pdu.mtch_active);
      LOG_D(
          MAC,
          "[eNB %d][MBMS USER-PLANE ] CC_id %d Generate header : sdu_length_total %d, num_sdus %d, sdu_lengths[0] %d, sdu_lcids[0] "
          "%d => payload offset %d,padding %d,post_padding %d (mcs %d, TBS %d), header MTCH %d, header MCCH %d, header MSI %d\n",
          module_idP,
          CC_id,
          sdu_length_total,
          num_sdus,
      ...
      trace_pdu(DIRECTION_DOWNLINK,
                ...
                module_idP,
                WS_M_RNTI,
                0xfffd, // M_RNTI = 6 in wirehsark
                RC.mac[module_idP]->frame,
                RC.mac[module_idP]->subframe,
                0,
                0);
...
int schedule_MBMS(module_id_t module_idP, uint8_t CC_id, frame_t frameP, sub_frame_t subframeP)
{
    ...
    for (i = 0; i < cc->num_active_mbsfn_area; i++) {
      ...
      ii = 0;
      LOG_D(MAC,
            "[eNB %d] CC_id %d Frame %d subframeP %d : Checking MBSFN Sync Area %d/%d with SF allocation %d/%d for MCCH and MTCH "
            "(mbsfn period %d, mcch period %d)\n",
            module_idP,
            CC_id,
            frameP,
            subframeP,
            i,
            cc->num_active_mbsfn_area,
            j,
            cc->num_sf_allocation_pattern,
            mbsfn_period,
            mcch_period);
    ...
    if (msi_flag == 1) {
       ...
       for (k = 0; k < num_mtch; k++) { // loop for all session in this MCH (MCH[0]) at this moment
        ...
        if (msi_pmch_stop >= num_sf_alloc /*&& msi_pmch_stop <=cc->pmch_Config[0]->sf_AllocEnd_r9*/) {
          ...
          ((MSI_ELEMENT *)msi_ptr)->stop_sf_LSB =
              (msi_pmch_stop <= cc->pmch_Config[0]->sf_AllocEnd_r9 ? msi_pmch_stop : cc->pmch_Config[0]->sf_AllocEnd_r9);
          msi_pmch_stop =
              (msi_pmch_stop <= cc->pmch_Config[0]->sf_AllocEnd_r9 ? msi_pmch_stop : cc->pmch_Config[0]->sf_AllocEnd_r9);
        } else {
          ((MSI_ELEMENT *)msi_ptr)->stop_sf_MSB = 0x7; // last subframeP of this mtch (only one mtch now)
       ...
       LOG_W(
           ...
           msi_pmch_stop,
           msi_sfs,
           cc->pmch_Config[0]->sf_AllocEnd_r9);
    ...
    if (mtch_flag == 1 ) {
       ...
       /* if ((msi_flag==1) || (mcch_flag==1)) {
          ...
          // get MTCH data from RLC (like for DTCH)
          LOG_D(MAC,"[eNB %d] CC_id %d Frame %d subframe %d: Schedule MTCH (area %d, sfAlloc
       ...
       if (rlc_status.bytes_in_buffer > 0 || msi_pmch_stop > 0 /*msi_pmch_stop>=num_sf_alloc*/) {
        ...
        // sdu_lengths[num_sdus] = mac_rlc_data_req(module_idP,frameP, MBMS_FLAG_NO,  MTCH+(MAX_NUM_RB*(MAX_MOBILES_PER_ENB+1)),
        // (char*)&mch_buffer[sdu_length_total]);
        LOG_I(MAC,
              "[eNB %d][MBMS USER-PLANE] CC_id %d Got %d bytes for MTCH %d msi_pmch_stop %d msi_sfs %d sdu_lengths[num_sdus] %d\n",
              module_idP,
        ...
        //    LOG_E(MAC,
        //      "[eNB %d][MBMS USER-PLANE] CC_id %d Got %d bytes for MTCH %d msi_pmch_stop %d msi_buffer %d msi_sfs %ld
        //      msi_buffer_act %ld  sdu_lengths[num_sdus] %d\n", module_idP, CC_id, sdu_lengths[num_sdus],
        //      MTCH,msi_pmch_stop,msi_sfs,msi_buffer_act, sdu_lengths[num_sdus]);
    ...
    if ((sdu_length_total + header_len_msi + header_len_mcch + header_len_mtch) > 0) {
       ...
       LOG_D(MAC,
             ...
             cc->MCH_pdu.mtch_active);
       LOG_D(MAC,
             "[eNB %d][MBMS USER-PLANE ] CC_id %d Generate header : sdu_length_total %d, num_sdus %d, sdu_lengths[0] %d, "
             "sdu_lcids[0] "
             "%d => payload offset %d,padding %d,post_padding %d (mcs %d, TBS %d), header MTCH %d, header MCCH %d, header MSI %d\n",
             module_idP,
             CC_id,
