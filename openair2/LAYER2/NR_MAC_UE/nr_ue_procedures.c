...
static int8_t nr_ue_process_dci(module_id_t module_id,
                                frame_t frame,
                                int slot,
                                dci_pdu_rel15_t *dci,
                                fapi_nr_dci_indication_pdu_t *dci_ind,
                                const nr_dci_format_t format)
{
  ...
  switch (format) {
    case NR_UL_DCI_FORMAT_0_0:
      return nr_ue_process_dci_ul_00(module_id, frame, slot, dci, dci_ind);
      break;

    case NR_UL_DCI_FORMAT_0_1:
      return nr_ue_process_dci_ul_01(module_id, frame, slot, dci, dci_ind);
      break;

    case NR_DL_DCI_FORMAT_1_0:
      return nr_ue_process_dci_dl_10(module_id, frame, slot, dci, dci_ind);
      break;

    case NR_DL_DCI_FORMAT_1_1:
      return nr_ue_process_dci_dl_11(module_id, frame, slot, dci, dci_ind);
      break;
...
nr_dci_format_t nr_ue_process_dci_indication_pdu(module_id_t module_id, frame_t frame, int slot, fapi_nr_dci_indication_pdu_t *dci)
{
  ...
  int ret = nr_ue_process_dci(module_id, frame, slot, def_dci_pdu_rel15, dci, format);
  if (ret < 0)
    return NR_DCI_NONE;
  return format;
}
                                           ...
                                           int slot)
{
  ...
  switch (dci_format) {
    case NFAPI_NR_FORMAT_0_0_AND_1_0:
      format = nr_extract_dci_00_10(mac, dci_size, rnti_type, dci_pdu, slot, N_RB);
      break;
