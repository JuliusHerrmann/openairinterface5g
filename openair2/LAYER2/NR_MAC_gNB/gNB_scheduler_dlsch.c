...
static void qos_aware_dl(module_id_t module_id,
                         frame_t frame,
                         sub_frame_t slot,
                         ...
                         uint16_t *rballoc_mask)
{
  ...
  // sort out the lcid depending on the lcid priority and data available from transmission buffer(only for DRBs and SRBs are always
  // given higher priority before DRBs)
...
static void scheduler_dl(module_id_t module_id,
                         frame_t frame,
                         sub_frame_t slot,
                         NR_UE_info_t **UE_list,
                         int max_num_ue,
                         ...
                         uint16_t *rballoc_mask)
{
  ...
  while (remainUEs > 0 && n_rb_sched >= min_rbSize && iterator->UE != NULL) {
    ...
    uint32_t num_bytes;
    if (false) {
      num_bytes = sched_ctrl->num_total_bytes;
    } else {
      // calculate the TBS as per the prioritized number of bytes in each LCID bytes
      uint32_t total_prioritized_bytes = nr_get_num_prioritized_bytes(max_rbSize, iterator, LCID_sched);
...
static void nr_fr1_dlsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  LOG_D(NR_MAC,
        ...
        nrOfSymbols,
        slbitmap);
  LOG_D(NR_MAC, "number of rbs scheduled are %d\n", n_rb_sched);
  LOG_D(NR_MAC, "slbitmap is %hu\n", slbitmap);
  LOG_D(NR_MAC, "\n");

  scheduler_dl(module_id, frame, slot, UE_info->list, max_sched_ues, n_rb_sched, rballoc_mask);
  /*
  pf_dl(module_id,
        frame,
        slot,
