...
{
  // printf("%s() SFN/SF:%d %s req:%p pdu_list:%p\n", __FUNCTION__, NFAPI_SFNSF2DEC(req->sfn_sf),
  // pnf_p7->_public.codec_config.deallocate ? "DEALLOCATE" : "FREE", req, req->dl_config_request_body.dl_config_pdu_list);
...
{
        // printf("%s() SFN/SF:%d %s req:%p pdu_list:%p\n", __FUNCTION__, NFAPI_SFNSF2DEC(req->sfn_sf),
        // pnf_p7->_public.codec_config.deallocate ? "DEALLOCATE" : "FREE", req, req->hi_dci0_request_body.hi_dci0_pdu_list);
        //  if(pnf_p7->_public.codec_config.deallocate)
        //  {
...
void deallocate_nfapi_tx_request(nfapi_tx_request_t* req, pnf_p7_t* pnf_p7)
{
        ...
        // pnf_p7->_public.codec_config.deallocate ? "DEALLOCATE" : "FREE", req,
        // req->tx_request_body.tx_pdu_list[i].segments[0].segment_data);
...
int pnf_p7_send_message(pnf_p7_t* pnf_p7, uint8_t* msg, uint32_t len)
{
    ...
    if ((sendto_result =
             sendto((int)pnf_p7->p7_sock, (const char*)msg, len, 0, (const struct sockaddr*)&remote_addr, remote_addr_len))
        < 0)
...
int pnf_p7_pack_and_send_p7_message(pnf_p7_t* pnf_p7, nfapi_p7_message_header_t* header, uint32_t msg_len)
{
    ...
    int len =
        nfapi_p7_message_pack(header, pnf_p7->tx_message_buffer, sizeof(pnf_p7->tx_message_buffer), &pnf_p7->_public.codec_config);
...
int pnf_nr_p7_pack_and_send_p7_message(pnf_p7_t* pnf_p7, nfapi_p7_message_header_t* header, uint32_t msg_len)
{
    ...
    int len = nfapi_nr_p7_message_pack(header,
                                       pnf_p7->tx_message_buffer,
                                       sizeof(pnf_p7->tx_message_buffer),
...
int pnf_p7_slot_ind(pnf_p7_t* pnf_p7, uint16_t phy_id, uint16_t sfn, uint16_t slot)
{
  ...
  if (pnf_p7->_public.slot_buffer_size != 0) // for now value is same as sf_buffer_size
  {
    ...
    // if (0) NFAPI_TRACE(NFAPI_TRACE_INFO, "%s() shift:%d slot_buffer->sfn_sf:%d tx_slot_buffer->sfn_slot:%d sfn_sf:%d
    // subframe_buffer[buffer_index:%u dl_config_req:%p tx_req:%p] "
    ...
    if (tx_slot_buffer->tx_data_req != 0 && tx_slot_buffer->tx_data_req->SFN == sfn_tx
        && tx_slot_buffer->tx_data_req->Slot == slot_tx) {
    ...
    if (tx_slot_buffer->dl_tti_req != 0 && tx_slot_buffer->dl_tti_req->SFN == sfn_tx
        && tx_slot_buffer->dl_tti_req->Slot == slot_tx) {
    ...
    if (tx_slot_buffer->ul_dci_req != 0 && tx_slot_buffer->ul_dci_req->SFN == sfn_tx
        && tx_slot_buffer->ul_dci_req->Slot == slot_tx) {
    ...
    if (rx_slot_buffer->dl_tti_req == 0 && rx_slot_buffer->tx_data_req == 0 && rx_slot_buffer->ul_tti_req == 0) {
    ...
    // printf("pnf_p7->_public.timing_info_mode_periodic:%d pnf_p7->timing_info_period_counter:%d
    // pnf_p7->_public.timing_info_period:%d\n", pnf_p7->_public.timing_info_mode_periodic, pnf_p7->timing_info_period_counter,
    // pnf_p7->_public.timing_info_period); printf("pnf_p7->_public.timing_info_mode_aperiodic:%d
    // pnf_p7->timing_info_aperiodic_send:%d\n", pnf_p7->_public.timing_info_mode_aperiodic, pnf_p7->timing_info_aperiodic_send);
  ...
  if (pnf_p7->_public.subframe_buffer_size != 0) {
    ...
    if (pnf_p7->sfn_sf_shift != 0) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "Applying shift %d to sfn/sf (%d -> %d)\n",
                  pnf_p7->sfn_sf_shift,
    ...
    if (0)
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() shift:%d subframe_buffer->sfn_sf:%d tx_subframe_buffer->sfn_sf:%d sfn_sf:%d "
                  "subframe_buffer[buffer_index:%u dl_config_req:%p tx_req:%p] "
                  "TX:sfn_sf:%d:tx_buffer_index:%d[dl_config_req:%p tx_req:%p]\n",
                  __FUNCTION__,
                  pnf_p7->sfn_sf_shift,
                  ...
                  sfn_sf_dec,
                  buffer_index,
                  subframe_buffer->dl_config_req,
                  subframe_buffer->tx_req,
                  tx_sfn_sf_dec,
                  tx_buffer_index,
                  tx_subframe_buffer->dl_config_req,
                  tx_subframe_buffer->tx_req);
    ...
    if (subframe_buffer->dl_config_req == 0 && subframe_buffer->tx_req == 0 && subframe_buffer->ul_config_req == 0
        && subframe_buffer->lbt_dl_config_req == 0 && subframe_buffer->ue_release_req == 0) {
...
uint8_t is_nr_p7_request_in_window(uint16_t sfn,uint16_t slot, const char* name, pnf_p7_t* phy)
{
  ...
  //         (current_sfn_slot_dec - recv_sfn_slot_dec));
  //     }
  ...
  //         NFAPI_TRACE(NFAPI_TRACE_NOTE, "[%d] %s is out of window %d (delta:%d) [max:%d]\n", current_sfn_slot_dec, name,
  //         recv_sfn_slot_dec,  (recv_sfn_slot_dec - current_sfn_slot_dec), timing_window);
  //     }
  ...
  if (current_sfn_slot_dec <= recv_sfn_slot_dec + timing_window) {
    ...
    NFAPI_TRACE(NFAPI_TRACE_NOTE,
                "[%d] %s is out of window %d (delta:%d) [max:%d]\n",
...
uint8_t is_p7_request_in_window(uint16_t sfnsf, const char* name, pnf_p7_t* phy)
{
  ...
  if (recv_sfn_sf_dec <= current_sfn_sf_dec) {
    ...
    if (((current_sfn_sf_dec + timing_window) % NFAPI_MAX_SFNSFDEC) < current_sfn_sf_dec) {
      ...
      // too late
      NFAPI_TRACE(NFAPI_TRACE_NOTE,
                  "[%d] %s is in late %d (delta:%d)\n",
                  current_sfn_sf_dec,
                  name,
                  recv_sfn_sf_dec,
                  (current_sfn_sf_dec - recv_sfn_sf_dec));
    }
  ...
  if (unpack_result == 0) {
    ...
    if (is_nr_p7_request_in_window(req->SFN, req->Slot, "dl_tti_request", pnf_p7)) {
      ...
      if (pnf_p7->slot_buffer[buffer_index].dl_tti_req != 0) {
        NFAPI_TRACE(NFAPI_TRACE_NOTE,
                    "%s() is_nr_p7_request_in_window()=TRUE buffer_index occupied - free it first sfn_slot:%d buffer_index:%d\n",
                    __FUNCTION__,
...
void pnf_handle_dl_config_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
  ...
  int unpack_result =
      nfapi_p7_message_unpack(pRecvMsg, recvMsgLen, req, sizeof(nfapi_dl_config_request_t), &(pnf_p7->_public.codec_config));
  ...
  if (unpack_result == 0) {
    ...
    if (0 && (NFAPI_SFNSF2DEC(req->sfn_sf) % 100 == 0 || NFAPI_SFNSF2DEC(req->sfn_sf) % 105 == 0))
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "DL_CONFIG.req sfn_sf:%d pdcch:%u dci:%u pdu:%u pdsch_rnti:%u pcfich:%u\n",
    ...
    if (is_p7_request_in_window(req->sfn_sf, "dl_config_request", pnf_p7)) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() %ld.%09ld POPULATE DL_CONFIG_REQ sfn_sf:%d buffer_index:%d\n",
                  __FUNCTION__,
                  t.tv_sec,
                  t.tv_nsec,
                  sfn_sf_dec,
      ...
      if (pnf_p7->subframe_buffer[buffer_index].dl_config_req != 0) {
        NFAPI_TRACE(NFAPI_TRACE_NOTE,
                    "%s() is_p7_request_in_window()=TRUE buffer_index occupied - free it first sfn_sf:%d buffer_index:%d\n",
                    __FUNCTION__,
                    NFAPI_SFNSF2DEC(req->sfn_sf),
                    buffer_index);
        // NFAPI_TRACE(NFAPI_TRACE_NOTE, "[%d] Freeing dl_config_req at index %d (%d/%d)",
...
void pnf_handle_ul_config_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
  ...
  int unpack_result =
      nfapi_p7_message_unpack(pRecvMsg, recvMsgLen, req, sizeof(nfapi_ul_config_request_t), &(pnf_p7->_public.codec_config));
  ...
  if (unpack_result == 0) {
    ...
    if (is_p7_request_in_window(req->sfn_sf, "ul_config_request", pnf_p7)) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() %ld.%09ld POPULATE UL_CONFIG_REQ sfn_sf:%d buffer_index:%d\n",
                  __FUNCTION__,
                  t.tv_sec,
                  t.tv_nsec,
                  sfn_sf_dec,
      ...
      NFAPI_TRACE(NFAPI_TRACE_NOTE,
                  "[%d] NOT storing ul_config_req OUTSIDE OF TRANSMIT BUFFER WINDOW SFN/SF %d\n",
                  NFAPI_SFNSF2DEC(pnf_p7->sfn_sf),
                  NFAPI_SFNSF2DEC(req->sfn_sf));
...

{
  ...
  int unpack_result =
      nfapi_p7_message_unpack(pRecvMsg, recvMsgLen, req, sizeof(nfapi_hi_dci0_request_t), &pnf_p7->_public.codec_config);
...
void pnf_handle_tx_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
  ...
  if (unpack_result == 0) {
    ...
    if (is_p7_request_in_window(req->sfn_sf, "tx_request", pnf_p7)) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() %ld.%09ld POPULATE TX_REQ sfn_sf:%d buffer_index:%d\n",
                  __FUNCTION__,
                  t.tv_sec,
                  t.tv_nsec,
                  sfn_sf_dec,
      ...
      if (0 && NFAPI_SFNSF2DEC(req->sfn_sf) % 100 == 0)
        NFAPI_TRACE(NFAPI_TRACE_INFO,
                    "%s() TX_REQ.req sfn_sf:%d pdus:%d - TX_REQ is within window\n",
                    __FUNCTION__,
                    NFAPI_SFNSF2DEC(req->sfn_sf),
                    req->tx_request_body.number_of_pdus);
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() TX_REQUEST Request is outside of window REQ:SFN_SF:%d CURR:SFN_SF:%d\n",
                  __FUNCTION__,
                  NFAPI_SFNSF2DEC(req->sfn_sf),
                  NFAPI_SFNSF2DEC(pnf_p7->sfn_sf));
...
void pnf_handle_lbt_dl_config_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
  ...
  int unpack_result =
      nfapi_p7_message_unpack(pRecvMsg, recvMsgLen, req, sizeof(nfapi_lbt_dl_config_request_t), &pnf_p7->_public.codec_config);
...
void pnf_handle_ue_release_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
  ...
  int unpack_result =
      nfapi_p7_message_unpack(pRecvMsg, recvMsgLen, req, sizeof(nfapi_ue_release_request_t), &pnf_p7->_public.codec_config);
  if (unpack_result == 0) {
    if (pthread_mutex_lock(&(pnf_p7->mutex)) != 0) {
    ...
    if (is_p7_request_in_window(req->sfn_sf, "ue_release_request", pnf_p7)) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() %ld.%09ld POPULATE UE_RELEASE_REQ sfn_sf:%d buffer_index:%d\n",
                  __FUNCTION__,
                  t.tv_sec,
                  t.tv_nsec,
                  sfn_sf_dec,
      ...
      if (0 && NFAPI_SFNSF2DEC(req->sfn_sf) % 100 == 0)
        NFAPI_TRACE(NFAPI_TRACE_INFO,
                    "%s() UE_RELEASE_REQ.req sfn_sf:%d rntis:%d - UE_RELEASE_REQ is within window\n",
                    __FUNCTION__,
                    NFAPI_SFNSF2DEC(req->sfn_sf),
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "%s() UE_RELEASE_REQUEST Request is outside of window REQ:SFN_SF:%d CURR:SFN_SF:%d\n",
                  __FUNCTION__,
                  NFAPI_SFNSF2DEC(req->sfn_sf),
                  NFAPI_SFNSF2DEC(pnf_p7->sfn_sf));
...
uint32_t calculate_t2(uint32_t now_time_hr, uint16_t sfn_sf, uint32_t sf_start_time_hr)
{
  ...
  if (0) {
    ...
    NFAPI_TRACE(NFAPI_TRACE_ERROR,
                "%s(now_time_hr:%u sfn_sf:%d sf_start_time_Hr:%u) sf_time_us:%u t2:%u prev_t2:%u diff:%u\n",
                __FUNCTION__,
                now_time_hr,
                NFAPI_SFNSF2DEC(sfn_sf),
                sf_start_time_hr,
...
uint32_t calculate_nr_t2(uint32_t now_time_hr, uint16_t sfn, uint16_t slot, uint32_t slot_start_time_hr)
{
  ...
  if (0) {
    ...
    NFAPI_TRACE(NFAPI_TRACE_ERROR,
                "%s(now_time_hr:%u sfn to slot:%d slot_start_time_Hr:%u) slot_time_us:%u t2:%u prev_t2:%u diff:%u\n",
                __FUNCTION__,
                now_time_hr,
...
void pnf_handle_p7_message(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7, uint32_t rx_hr_time)
{
  ...
  // unpack the message header
  if (nfapi_p7_message_header_unpack(pRecvMsg,
                                     recvMsgLen,
                                     &messageHeader,
                                     sizeof(nfapi_p7_message_header_t),
                                     &pnf_p7->_public.codec_config)
      < 0) {
    NFAPI_TRACE(NFAPI_TRACE_ERROR, "Unpack message header failed, ignoring\n");
    return;
      ...
      for (i = 0; i < rx_msg->num_segments_expected; ++i) {
        if (i == 0) {
          ...
          memcpy(pnf_p7->reassemby_buffer + offset,
                 rx_msg->segments[i].buffer + NFAPI_P7_HEADER_LENGTH,
                 rx_msg->segments[i].length - NFAPI_P7_HEADER_LENGTH);
