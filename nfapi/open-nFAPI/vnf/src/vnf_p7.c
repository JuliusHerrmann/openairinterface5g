    ...
    if (len > vnf_p7->_public.segment_size) {
      ...
      for (segment = 0; segment < segment_count; ++segment) {
        ...
        nfapi_p7_update_transmit_timestamp(
            buffer,
            calculate_transmit_timestamp(p7_connection->sfn, p7_connection->slot, vnf_p7->slot_start_time_hr));
...
void vnf_handle_ul_node_sync(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7)
{
  ...
  if (!(phy->filtered_adjust)) {
    ...
    NFAPI_TRACE(NFAPI_TRACE_NOTE,
                "(%4d/%d) PNF to VNF !sync phy_id:%d (t1/2/3/4:%8u, %8u, %8u, %8u) txrx:%4u procT:%3u latency(us):%4d\n",
                NFAPI_SFNSF2SFN(phy->sfn_sf),
                NFAPI_SFNSF2SF(phy->sfn_sf),
                ind.header.phy_id,
                ind.t1,
                ind.t2,
                ind.t3,
    ...
    if (1) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_INFO,
                  "(%4d/%1d) %ld.%ld PNF to VNF phy_id:%2d (t1/2/3/4:%8u, %8u, %8u, %8u) txrx:%4u procT:%3u "
                  "latency(us):%4d(avg:%4d) offset(us):%8d filtered(us):%8d wrap[t1:%u t2:%u]\n",
                  NFAPI_SFNSF2SFN(phy->sfn_sf),
                  NFAPI_SFNSF2SF(phy->sfn_sf),
                  ts.tv_sec,
                  ts.tv_nsec,
                  ...
                  (ind.t1 < phy->previous_t1),
                  (ind.t2 < phy->previous_t2));
    }
  }
        ...
        if (phy->min_sync_cycle_count == 0) {
...

          {
            ...
            NFAPI_TRACE(NFAPI_TRACE_INFO,
                        "PNF to VNF phy_id:%d adjustment%d phy->previous_sf_offset_filtered:%d phy->previous_sf_offset_filtered:%d "
                        "phy->sf_offset_trend:%d\n",
                        ind.header.phy_id,
                        phy->adjustment,
                        phy->previous_sf_offset_filtered,
                        phy->previous_sf_offset_filtered,
                        phy->sf_offset_trend);
              ...
              if (phy->adjustment == 0) {
                ...
                if (phy->in_sync) {
                  ...
                  if (phy->insync_minor_adjustment != 0) {
                    NFAPI_TRACE(NFAPI_TRACE_NOTE,
                                "(%4d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d subframes "
                                "(sf_offset_filtered:%d) %d %d %d NEW:%d CURR:%d adjustment:%d\n",
                                NFAPI_SFNSF2SFN(phy->sfn_sf),
                                NFAPI_SFNSF2SF(phy->sfn_sf),
                                ind.header.phy_id,
                  ...
                  if (phy->adjustment == 0) {
                    ...
                    // if(phy->adjustment == 1)
                    {
                      ...
                      if (phy->sf_offset_filtered > 250) {
                        ...
                        phy->insync_minor_adjustment = -(insync_minor_adjustment_2);
                        phy->insync_minor_adjustment_duration =
                            2 * ((phy->sf_offset_filtered + 250) / -(insync_minor_adjustment_2));
                      }
                    }
                    // else
                    {
                    ...
                    NFAPI_TRACE(NFAPI_TRACE_NOTE,
                                "(%4d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d subframes (adjustment:%d "
                                "sf_offset_filtered:%d) %d %d %d NEW:%d CURR:%d adj:%d\n",
                                NFAPI_SFNSF2SFN(phy->sfn_sf),
                                NFAPI_SFNSF2SF(phy->sfn_sf),
                                ind.header.phy_id,
                                phy->insync_minor_adjustment,
                    ...
                    NFAPI_TRACE(NFAPI_TRACE_NOTE,
                                "(%d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d subframes (adjustment:%d "
                                "sf_offset_filtered:%d) %d %d %d\n",
                                NFAPI_SFNSF2SFN(phy->sfn_sf),
                                NFAPI_SFNSF2SF(phy->sfn_sf),
                                ind.header.phy_id,
                                phy->insync_minor_adjustment,
            ...
            if (phy->in_sync == 0) {
              /*NFAPI_TRACE(NFAPI_TRACE_NOTE, "***** Adjusting VNF phy_id:%d SFN/SF (%s) from %d to %d (%d) mode:%s zeroCount:%u
                 sync:%s\n", ind.header.phy_id, (phy->in_sync ? "via sfn" : "now"), NFAPI_SFNSF2DEC(curr_sfn_sf),
                 NFAPI_SFNSF2DEC(new_sfn_sf), phy->adjustment, phy->filtered_adjust ? "FILTERED" : "ABSOLUTE", phy->zero_count,
...
void vnf_nr_handle_ul_node_sync(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7)
{
        ...
        if (nfapi_nr_p7_message_unpack(pRecvMsg, recvMsgLen, &ind, sizeof(nfapi_nr_ul_node_sync_t), &vnf_p7->_public.codec_config)
            < 0) {
          NFAPI_TRACE(NFAPI_TRACE_ERROR, "Failed to unpack ul_node_sync\n");
          return;
        }
        ...
        uint32_t tx_2_rx = t4 > ind.t1 ? t4 - ind.t1
                                       : t4 + NFAPI_MAX_SFNSLOTDEC
                                             - ind.t1; // time taken to receive ul node sync - time taken to send dl node sync
        uint32_t pnf_proc_time = ind.t3 - ind.t2;
        ...
        if (!(phy->filtered_adjust)) {
          ...
          // NFAPI_TRACE(NFAPI_TRACE_NOTE, "(%4d/%d) PNF to VNF !sync phy_id:%d (t1/2/3/4:%8u, %8u, %8u, %8u) txrx:%4u procT:%3u
          // latency(us):%4d\n",
          ...
          if (1) {
            ...
            // NFAPI_TRACE(NFAPI_TRACE_NOTE, "(%4d/%1d) %d.%d PNF to VNF phy_id:%2d (t1/2/3/4:%8u, %8u, %8u, %8u) txrx:%4u procT:%3u
            // latency(us):%4d(avg:%4d) offset(us):%8d filtered(us):%8d wrap[t1:%u t2:%u]\n",
        ...
        if (phy->min_sync_cycle_count == 0) {
          ...
          if (!phy->filtered_adjust) {
            ...
            NFAPI_TRACE(NFAPI_TRACE_NOTE,
                        "PNF to VNF slot offset:%d sfn :%d slot:%d \n",
                        phy->slot_offset,
                        NFAPI_SFNSLOTDEC2SFN(sfn_slot_dec),
                        NFAPI_SFNSLOTDEC2SLOT(sfn_slot_dec));
...

          {
            ...
            // NFAPI_TRACE(NFAPI_TRACE_NOTE, "PNF to VNF phy_id:%d adjustment%d phy->previous_slot_offset_filtered:%d
            // phy->previous_slot_offset_filtered:%d phy->slot_offset_trend:%d\n", ind.header.phy_id, phy->adjustment,
            // phy->previous_slot_offset_filtered, phy->previous_slot_offset_filtered, phy->slot_offset_trend);
            ...
            if (!phy->filtered_adjust) {
              ...
              if (phy->adjustment == 0) {
                ...
                if (phy->in_sync) {
                  ...
                  if (phy->insync_minor_adjustment != 0) {
                    // NFAPI_TRACE(NFAPI_TRACE_NOTE, "(%4d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d slots
                    // (slot_offset_filtered:%d) %d %d %d NEW:%d CURR:%d adjustment:%d\n",
                    //             phy->sfn, phy->slot, ind.header.phy_id,
                    //             phy->insync_minor_adjustment, phy->insync_minor_adjustment_duration,
                    //                                                     phy->slot_offset_filtered,
                    //                                                     insync_minor_adjustment_1, insync_minor_adjustment_2,
                    //                                                     phy->slot_offset_trend, NFAPI_SFNSLOT2DEC(new_sfn,
                    //                                                     new_slot), NFAPI_SFNSLOT2DEC(curr_sfn, curr_slot),
                    //                                                     phy->adjustment);
                  }
                }
              } else {
                  ...
                  if (phy->adjustment == 0) {
                    ...
                    // if(phy->adjustment == 1)
                    {
                      ...
                      if (phy->slot_offset_filtered > 250) {
                        ...
                        phy->insync_minor_adjustment = -(insync_minor_adjustment_2);
                        phy->insync_minor_adjustment_duration =
                            2 * ((phy->slot_offset_filtered + 250) / -(insync_minor_adjustment_2));
                      }
                    }
                    // else
                    {
                    ...
                    // NFAPI_TRACE(NFAPI_TRACE_NOTE, "(%4d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d slots
                    // (adjustment:%d slot_offset_filtered:%d) %d %d %d NEW:%d CURR:%d adj:%d\n",
                    //             phy->sfn, phy->slot, ind.header.phy_id,
                    //             phy->insync_minor_adjustment, phy->insync_minor_adjustment_duration, phy->adjustment,
                    //             phy->slot_offset_filtered, insync_minor_adjustment_1, insync_minor_adjustment_2,
                    ...
                    // NFAPI_TRACE(NFAPI_TRACE_NOTE, "(%d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d slots
                    // (adjustment:%d slot_offset_filtered:%d) %d %d %d\n",
                    //             phy->sfn, phy->slot, ind.header.phy_id,
                    //             phy->insync_minor_adjustment, phy->insync_minor_adjustment_duration, phy->adjustment,
                    //             phy->slot_offset_filtered, insync_minor_adjustment_1, insync_minor_adjustment_2,
                    //             phy->slot_offset_trend);
...
void vnf_nr_handle_timing_info(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7)
{
        ...
        if (nfapi_nr_p7_message_unpack(pRecvMsg, recvMsgLen, &ind, sizeof(nfapi_timing_info_t), &vnf_p7->_public.codec_config)
            < 0) {
          NFAPI_TRACE(NFAPI_TRACE_ERROR, "Failed to unpack timing_info\n");
          return;
        }
        ...
        if (vnf_p7 && vnf_p7->p7_connections)
        {
          ...
          //NFAPI_TRACE(NFAPI_TRACE_INFO, "%s() PNF:SFN/SF:%d VNF:SFN/SF:%d deltaSFNSF:%d\n", __FUNCTION__, NFAPI_SFNSF2DEC(ind.last_sfn_sf), NFAPI_SFNSF2DEC(vnf_p7->p7_connections[0].sfn_sf), vnf_pnf_sfnsf_delta);
            ...
            NFAPI_TRACE(NFAPI_TRACE_INFO,
                        "%s() LARGE SFN/SLOT DELTA between PNF and VNF. Delta %d. PNF:%d.%d VNF:%d.%d\n\n\n\n\n\n\n\n\n",
                        __FUNCTION__,
                        vnf_pnf_sfnslot_delta,
                        ind.last_sfn,
                        ind.last_slot,
                        vnf_p7->p7_connections[0].sfn,
                        vnf_p7->p7_connections[0].slot);
            // Panos: Careful here!!! Modification of the original nfapi-code
...
void vnf_handle_p7_message(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7) 
{
        ...
        if (pRecvMsg == NULL || recvMsgLen < 4 || vnf_p7 == NULL) {
            NFAPI_TRACE(NFAPI_TRACE_ERROR,
                        "vnf_handle_p7_message: invalid input params (%p %d %p)\n",
                        pRecvMsg,
                        recvMsgLen,
                        vnf_p7);
            return;
