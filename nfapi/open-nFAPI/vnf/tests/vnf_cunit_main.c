      ...
      for (phy_idx = 0; phy_idx < 4; ++phy_idx) {
        ...
        if (phy_info->enabled) {
          if (FD_ISSET(phy_info->p7_rx_sock, &rdfs)) {
            ...
            int len =
                recvfrom(phy_info->p7_rx_sock, &buffer[0], buffer_size, 0, 0, 0); // (struct sockaddr*)&recv_addr, &recv_addr_size);
            ...
            switch (header.message_id) {
              case NFAPI_DL_NODE_SYNC: {
                ...
                // send ul node sycn
                send_p7_segmented_msg(phy_info->p7_tx_sock,
                                      &buffer[0],
                                      len,
                                      segment_size,
                                      (struct sockaddr*)&phy_info->p7_tx_sockaddr,
                                      sizeof(phy_info->p7_tx_sockaddr));
              } break;
              case NFAPI_DL_CONFIG_REQUEST: {
                ...
                harq_ind.harq_indication_body.harq_pdu_list = (nfapi_harq_indication_pdu_t*)(malloc(
                    sizeof(nfapi_harq_indication_pdu_t) * harq_ind.harq_indication_body.number_of_harqs));
                ...
                // sendto(phy_info->p7_tx_sock, &buffer[0], len, 0, (struct sockaddr*)&phy_info->p7_tx_sockaddr,
                // sizeof(phy_info->p7_tx_sockaddr));
                send_p7_segmented_msg(phy_info->p7_tx_sock,
                                      &buffer[0],
                                      len,
...
int main ( void )
{
   ...
   while (s) {
      ...
      while (t) {
        ...
        while (failures) {
          if (strcmp(failures->pSuite->pName, s->pName) == 0 && strcmp(failures->pTest->pName, t->pName) == 0) {
            pass = 0;
            failures = 0;
          } else {
